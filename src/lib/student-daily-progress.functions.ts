...
export const studentDailyProgress = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    // Paginated readers — return ALL pages, fetched sequentially within each
    // reader but in parallel across readers via Promise.all below.
    const fetchAllMcqs = async () => {
      const out: Array<{ id: string; chapter_id: string | null }> = [];
      for (let from = 0; from < 100_000; from += PAGE) {
        ...
        out.push(...((data ?? []) as Array<{ id: string; chapter_id: string | null }>));
    ...
    const fetchPracticeProgress = async () => {
      ...
      for (let from = 0; from < 50_000; from += PAGE) {
        const { data, error } = await supabase
          ...
          .range(from, from + PAGE - 1);
        if (error) throw error;
        out.push(...((data ?? []) as PracticeProgressRow[]));
        if (!data || data.length < PAGE) break;
      }
      return out;
    };
    ...
    const fetchNonPracticeAnswers = async () => {
      const out: AttemptAnswerRow[] = [];
      for (let from = 0; from < 50_000; from += PAGE) {
        const { data, error } = await supabase
          ...
          .range(from, from + PAGE - 1);
        if (error) throw error;
        out.push(...((data ?? []) as unknown as AttemptAnswerRow[]));
        if (!data || data.length < PAGE) break;
      }
      return out;
    };
